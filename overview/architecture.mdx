...
<Note>
  Timeseries implements this pattern for OpenTelemetry metrics by leveraging the
  [`opendata` Buffer](/buffer/index) library via the `opendata` OTel Collector
  exporter and an in-server buffer consumer. See
