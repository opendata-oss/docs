...
These are the published benchmark results for OpenData Log. All numbers come
from real traffic on a single `m5n.xlarge` instance running against AWS S3, with
the event distribution randomly sampled by key. They first appeared in the
launch post,
[Kafka's Broken Promise: There is No Goldilocks Log](https://www.opendata.dev/blog/announcing-opendata-log).
...
25 MiB/s of throughput. The workaround for the tail is scoping read replicas to
smaller key ranges so the cache isn't thrashed by blocks from unrelated ranges.

### Range vs. random sharding

Scoping a reader to a contiguous key range improves cache locality versus
hash/random sharding. At a fixed 25 MB/s ingest over 1M keys, sweeping the
parallelism factor:
...
random sharding degrades.

## Cost

One fully-loaded read-serving node, with 1 day of retention:
...
That box buys 20 MB/s ingest, 50,000 concurrent followers, and 1M keys. Compute
is the whole bill; S3 is a rounding error. GETs for your read pattern sit
outside the total since most reads hit the 8 GB cache and stay
workload-dependent. Add a reader for another ~50k followers at +$174/mo.
...
Poll latencies scale with the rate of ingestion: higher ingest evicts hot data
before compaction can collocate keys for efficient querying. Log also inherits
object-storage latency floors. The mitigation is range-scoped read replicas, as
shown above.
...
- Scaling without data shuffles: [SlateDB clones](https://slatedb.io/docs/design/clones/)
