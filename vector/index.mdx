...
A truly stateless architecture is what makes Vector cheaper, more reliable, and
easier to operate than earlier vector databases. Any node can serve any data, so
...
hop-by-hop reads a graph index requires, which is what keeps cold queries fast
despite object-store latency.
