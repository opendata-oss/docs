...
OpenData is a collection of operationally simple, open source databases that
share a unified storage foundation on object storage.

OpenData is self-hosted software, not a hosted data API. You run the relevant
database binary on your own infrastructure, point it at S3-compatible object
storage, and call it from your application over HTTP. Buffer is the exception:
it is an embedded library for building high-throughput ingestion pipelines.

## Use OpenData from applications

- **Vector** is for embeddings, semantic search, and full-text search. Start
  with the [Vector quickstart](/vector/quickstart) or the
  [Vector OpenAPI spec](/openapi/vector.yaml).
- **Timeseries** is for Prometheus-compatible metrics. Start with the
  [Timeseries quickstart](/timeseries/quickstart) or the
  [Timeseries OpenAPI spec](/openapi/timeseries.yaml).
- **Log** is for keyed event streams. Start with the
  [Log quickstart](/log/quickstart) or the [Log OpenAPI spec](/openapi/log.yaml).
- **Buffer** is for durable ingestion queues on object storage. Start with the
  [Buffer docs](/buffer).
...
additional database system since each ships with its own implementations.

Is there a better way? We believe that the general availability of object
